                            ...
                            <table class="table-auto w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="bg-gray-900 divide-y divide-gray-700">
                                    @foreach ($tasks as $task)
                                        <tr class="h-16">
                                            <td class="px-6 py-4 whitespace-nowrap text-white">{{ $task->id }}</td>
                                            ...
                                            <td
                                                ...
                                                <div class="flex space-x-1">
                                                    <a href="{{ route('tasks.show', $task->id) }}"
                                                        class="w-6 h-6 flex items-center justify-center text-xs text-white bg-orange-500 rounded-md hover:bg-orange-600">S</a>
                                                    <a href="{{ route('tasks.formEdit', $task->id) }}"
                                                        class="w-6 h-6 flex items-center justify-center text-xs text-white bg-blue-500 rounded-md hover:bg-blue-600">M</a>
                                                </div>
                                                ...
                                                <div class="flex space-x-1">
                                                    <button
                                                        ...
                                                        class="w-6 h-6 flex items-center justify-center text-xs text-white bg-red-500 rounded-md hover:bg-red-600">D</button>
                                                    <form method="POST"
                                                        action="{{ route('tasks.complete', $task->id) }}">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                                            class="w-6 h-6 flex items-center justify-center text-xs text-white bg-green-500 rounded-md hover:bg-green-600">C</button>
                                                    </form>
                                                </div>
                                            </td>
                                        </tr>
                                    ...
                                    @for ($i = 0; $i < $rowsToAdd; $i++)
                                        <tr class="h-16">
                                            ...
                                            <td
                                                ...
                                                <!-- Fila superior (S y M) -->
                                                <div class="flex space-x-1">
                                                    <a class="w-6 h-6 flex items-center justify-center text-xs text-gray-400 bg-gray-700 rounded-md cursor-not-allowed"
                                                        disabled>-</a>
                                                    <a class="w-6 h-6 flex items-center justify-center text-xs text-gray-400 bg-gray-700 rounded-md cursor-not-allowed"
                                                        disabled>-</a>
                                                </div>
        ...
        <div x-show="showDeleteModal"
            ...
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 w-96">
                ...
                <div class="mt-4 flex justify-end">
                    ...
                    <form method="POST" :action="'/tasks/' + taskToDelete" class="ml-2">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
